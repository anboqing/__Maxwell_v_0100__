...
void* Thread::thread_func(void *arg){
    ...
    LogDebug("%s","Thread::threadfunc");
    p_thread->workking();
    return NULL;
}

void Thread::start(){
    LogDebug("%s","Thread::start");
    //创建线程 将本线程的指针传给线程启动函数 让它能调用 workking 函数开始工作
    // 第一个参数 将新建线程的线程id 赋值给了_tid成员
    pthread_t id;
    int ret = pthread_create(&id,NULL,thread_func,this);
    setThreadId(id);
    if(ret!=0){
